...
<head>
    ...
    <script type="text/javascript">
        ...
        function studentbukaochaxun(student_number) {
            ...
            $('#studentbukaoinfo').append(
               /* '<form action="insertuser" method="post">' +*/
                '<table id="studentcountchaxuntable" align="center" style="margin-top: 5%;color: black;text-align: center" border="1px" width="75%">' +
                    '<tr>' +
                        '<td colspan="4">' +
                            '<div style="width: 30%;height: 40%;margin-top: 3%;margin-left: 35%">' + '<img id="xueshengphoto" src="" width="80%">' + '</div>' +
                            '<div style="float: left;margin-top: 2%;margin-left: 30%;margin-bottom: 2%">' + '学号： ' +  '</div>' +
                            '<div id="student_number" style="float: left;margin-top: 2%;margin-left: 1%">' + student_number +  '</div>' +
                            '<div style="float: left;margin-top: 2%;margin-left: 3%">' + '姓名： ' + '</div>' +
                            '<div id="studentname" style="float: left;margin-top: 2%;margin-left: 1%">' + '</div>' +
                        '</td>' +
                    '</tr>' +
                    '<tr>' +
                        '<td>' + '语文' + '</td>' +
            ...
            $.ajax({
                ...
                url: "count/studentbukaoxinxichaxun",
                type: "post",
                data: {"student_number":student_number},
                dataType: "text",
                success:function (data) {

                    alert("学生补考信息查询OK！" + data);

                    var studentcountinfoobj = JSON.parse(data)
                    
                    /*$.each(studentcountinfoobj,function (index,studentcount) {*/

                        $("#studentcountchaxuntable").last().append(

                            '<tr>' +
                                '<td id="countyuwen">' + studentcountinfoobj.yuwen + '</td>' +
                                '<td id="countshuxue">' + studentcountinfoobj.shuxue + '</td>' +
                                '<td id="countyingyu">' + studentcountinfoobj.yingyu + '</td>' +
                                '<td id="counttiyu">' + studentcountinfoobj.tiyu + '</td>' +
                            '</tr>'

                        );

                        $("#studentname").text(studentcountinfoobj.name);

                        $("#xueshengphoto").attr("src",studentcountinfoobj.image);

                        if($("#countyuwen").text() < 70){

                            $("#countyuwen").text("需补考");

                        }
                        if($("#countshuxue").text() < 60){

                            $("#countshuxue").text("需补考");

                        }
                        if($("#countyingyu").text() < 70){

                            $("#countyingyu").text("需补考");

                        }
                        if($("#counttiyu").text() < 90) {
...
<div data-options="region:'west',iconCls:'icon-large-smartart',title:'菜单栏',split:true" style="width:200px;">
    ...
    <div id="aa" class="easyui-accordion" style="width:300px;height:200px" data-options="fit:true">
        ...
        <div title="学生个人情况管理" data-options="iconCls:'icon-redo'" style="overflow:auto;padding:10px;">
            <a id="btn1" href="###" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="studentbukaochaxun(${username})">学生课程补考查询</a>
        </div>
